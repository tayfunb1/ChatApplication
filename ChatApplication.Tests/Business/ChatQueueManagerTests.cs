...
public class ChatQueueManagerTests
{
    private readonly ChatQueueManager _manager = new();
    ...
    public async Task DequeueChatSessionWithCondition_Should_Return_Null_If_Condition_Does_Not_Match()
    {
        ...
        await _manager.QueueChatSession(session1, team2);
        await _manager.QueueChatSession(session2, team2);

        // Act
        var result = await _manager.DequeueChatSessionWithCondition(x => x.Id == Guid.NewGuid());
